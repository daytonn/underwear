...
describe("String extensions", function() {
  ...
  describe("inflection", function() {
    ...
    describe("pluralize", function() {
      it("doesn't pluralize uncountable words", function() {
        uncountable.each(function(word) {
          expect(word.pluralize()).to.equal(word);
        });
      });

      it("pluralizes words", function() {
        words.each(function(pair) {
          expect(pair[0].pluralize()).to.equal(pair[1]);
        });
      });

      it("singularizes when count given is 1", function() {
        words.each(function(pair) {
          expect(pair[0].pluralize(1)).to.equal(pair[0]);
        });
      });

      it("pluralizes when count given is greater than 1", function() {
        words.each(function(pair) {
          expect(pair[0].pluralize(5)).to.equal(pair[1]);
        });
      });
    });

    describe("singularize", function() {
      it("doesn't singularize uncountable words", function() {
        uncountable.each(function(word) {
      ...
      it("singularizes words", function() {
        words.each(function(pair) {
          expect(pair[1].singularize()).to.equal(pair[0]);
        });
      });
    });
  });
